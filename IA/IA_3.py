...
class IA_Bomber:
    def __init__(
        ...
        def get_minerais(self, map):
            """Retourne la liste des minerais les plus proches."""
            minerais = []
            ...
            minerais = sorted(
                minerais, key=lambda m: self.get_min_distance(self.position, m)
            )[:10]
            return minerais
        ...
        def get_neighbors(self, pos, map):
            """Retourne les positions voisines accessibles (non bloquées par un mur)."""
            neighbors = []
        ...
        def find_path(self, start, goal, map):
            """Recherche un chemin jusqu’à la position goal en évitant les fantômes."""
            from collections import deque

            frontier = deque([(start, [start])])
            visited = {start}
            ghost_positions = [
                g["position"] for g in game_dict["fantômes"]
            ]  # Récup. fantômes
            ...
            while frontier:
                ...
                for next_pos in self.get_neighbors(self, pos, map):
                    ...
                    if any(
                        self.get_min_distance(next_pos, gp) <= 2
        ...
        def heuristic(self, pos1, pos2):
            """Renvoie l’estimation de la distance entre deux positions."""
            return abs(pos1[0] - pos2[0]) + abs(pos1[1] - pos2[1])

        self.heuristic = heuristic

        def get_direction(self, current_pos, next_pos):
            """Détermine la direction de déplacement entre deux positions adjacentes."""
        ...
        def find_safe_spot(self, pos, map):
            """Trouve un emplacement sûr pour se protéger."""
            # Check positions at least 3 cells away for better safety
            safe_spots = []
            search_range = 5  # Augmenté à 5
            for y in range(
                max(0, pos[1] - search_range), min(len(map), pos[1] + search_range + 1)
            ):
                for x in range(
                    max(0, pos[0] - search_range),
                    min(len(map[0]), pos[0] + search_range + 1),
                ):
                    if (
                        map[y][x] != "C"
                        and (x, y) != pos
                        and abs(x - pos[0]) + abs(y - pos[1]) >= 5
                    ):  # Augmenté à 5
                        safe_spots.append((x, y))
            return min(
                safe_spots, key=lambda p: self.get_min_distance(pos, p), default=None
            )

        self.find_safe_spot = find_safe_spot

        def predict_ghost_positions(self, ghosts, current_pos):
            """Anticipe les positions des fantômes pour éviter les zones dangereuses."""
            danger_zones = set()
            for ghost in ghosts:
                ...
                for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
                    ...
                    if (
                        0 <= next_y < len(self.map)
                        and 0 <= next_x < len(self.map[0])
                        and self.map[next_y][next_x] != "C"
                    ):
                        danger_zones.add((next_x, next_y))
            return danger_zones

        def is_ghost_nearby(self, pos, ghosts, safe_distance=4):
            """Vérifie si un fantôme est suffisamment proche pour constituer une menace."""
            for ghost in ghosts:
                ghost_pos = ghost["position"]
        ...
        def is_position_safe(self, pos, game_dict):
            """Contrôle si la position est sûre vis-à-vis des fantômes et des bombes."""
            # Vérification immédiate des fantômes
            for ghost in game_dict["fantômes"]:
                if (
                    self.get_min_distance(pos, ghost["position"]) <= 4
        ...
        def find_safest_escape(self, pos, game_dict):
            """Détermine la position la plus sûre pour s’enfuir."""
            safe_spots = []
            checked = set()
            to_check = [(pos, 0)]
                ...
                if is_safe:
                    for bomber in game_dict["bombers"]:
                        if bomber["num_joueur"] != self.num_joueur and bomber["pv"] > 0:
                            if (
                                self.get_min_distance(current_pos, bomber["position"])
                                <= 2
                            ):
                                is_safe = False
                                break
            ...
            if safe_spots:
                ...
                def get_safety_score(spot):
                    ...
                    if game_dict["fantômes"]:
                        ghost_distance = min(
                            self.get_min_distance(spot[0], g["position"])
                    ...
                    if active_bombers:
                        bomber_distance = min(
                            self.get_min_distance(spot[0], b["position"])
                            for b in active_bombers
                        )

                    return (
                        ghost_distance  # On valorise surtout l'éloignement des fantômes
                    )
        ...
        def can_hit_ghost(self, pos, ghost_pos):
            """Vérifie si un fantôme peut être touché (distance de 3 case)."""
            # Vérifie si un fantôme est à exactement 3 case de distance
            return self.get_min_distance(pos, ghost_pos) == 3

        def is_safe_to_bomb(self, pos, game_dict):
            """Estime s’il est sans danger de poser une bombe à cette position."""
        ...
        def is_ghost_pursuing(self, pos, ghost_pos, prev_ghost_positions):
            """Vérifie si un fantôme se rapproche tour après tour."""
        ...
        def find_defensive_position(self, pos, ghost_pos, game_dict):
            """Localise une position pour poser un dispositif défensif."""
            ...
            for neighbor in self.get_neighbors(self, pos, game_dict["map"]):
                ...
                if self.get_min_distance(
                    neighbor, ghost_pos
                ) == 1 and self.is_safe_to_bomb(self, neighbor, game_dict):
        ...
        def has_line_of_sight(self, start, end, map):
            """Vérifie si une ligne droite entre deux positions est bloquée par un mur."""
            ...
            for _ in range(n):
                ...
                if (
                    map[int(cy)][int(cx)] == "C"
                ):  # Renvoie False si un mur bloque la vue
                    return False
            return True
    ...
    def action(self, game_dict: dict) -> str:
        ...
        for ghost in game_dict["fantômes"]:
            ...
            ) <= 3 and self.is_ghost_pursuing(
                self, self.position, ghost_pos, self.prev_ghost_positions
            ):
        ...
        if pursuing_ghost:
            defensive_pos = self.find_defensive_position(
                self, self.position, pursuing_ghost["position"], game_dict
            )
            if defensive_pos:
        ...
        if game_dict["fantômes"]:
            ...
            if closest_ghost_dist <= 4:  # Si un fantôme est trop proche
                ...
                if safe_spot:
                    escape_path = self.find_path(
                        self, self.position, safe_spot, game_dict["map"]
                    )
                ...
                for neighbor in self.get_neighbors(
                    self, self.position, game_dict["map"]
                ):
                ...
                if best_move and max_distance > self.get_min_distance(
                    self.position, ghost_pos
                ):
        ...
        if game_dict["fantômes"]:
            for ghost in game_dict["fantômes"]:
                ghost_pos = ghost["position"]
                if self.can_hit_ghost(
                    self, self.position, ghost_pos
                ) and self.is_safe_to_bomb(self, self.position, game_dict):
                    self.just_bombed = True
                    self.bomb_timer = 0
                    return "X"
        ...
        for minerai in minerais:
            if self.get_min_distance(
                self.position, minerai
            ) == 1 and self.is_position_safe(self, self.position, game_dict):
                self.just_bombed = True
                self.bomb_timer = 0
                self.safe_path = []
                return "X"
        ...
        if not self.current_path or self.path_index >= len(self.current_path):
            ...
            temp_path = self.find_path(
                self, self.position, closest_minerai, game_dict["map"]
            )
            # Filter out unsafe positions
            safe_path = [
                pos for pos in temp_path if self.is_position_safe(self, pos, game_dict)
            ]
                ...
                if safe_spot:
                    ...
                    if len(self.safe_path) > 1:
                        return self.get_direction(
                            self, self.position, self.safe_path[1]
                        )
                return "N"

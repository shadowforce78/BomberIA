...
class IA_Bomber:
    ...
    def flood_fill(self, game_dict: dict) -> list[tuple[tuple[int, int], int]]:
        ...
        minerais.sort(key=lambda m: (m[1], abs(m[0][0]-center_x) + abs(m[0][1]-center_y)))
    ...
    def can_move_to(self, pos: tuple[int, int], direction: str, game_dict: dict) -> bool:
        ...
        for fantome in game_dict.get("fantômes", []):
            if fantome["position"] == (new_x, new_y):
                return False
    ...
    def action(self, game_dict: dict) -> str:
        ...
        if ((dist_x <= 1 and dist_y == 0) or (dist_y <= 1 and dist_x == 0)):
            ...
            if self.get_safe_direction(bomber_pos, game_dict["bombes"], game_dict["map"]) != "N":

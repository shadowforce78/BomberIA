...
Action = Literal[
    "H", "B", "G", "D", "X", "N"
]  # haut, bas, gauche, droite, bombe, passe
Map = list[list[str]]
...
class Fantôme:
    ...
    def __init__(self, pos: Position):
        self.position: Position = pos
        self.case_précédente: Position | None = (
            None  # case du fantôme au tour précédent
        )
        self.pv = 1
    ...
    def to_dict(self):
        return {
            "position": self.position.to_dict(),
            "case prec": (
                self.case_précédente.to_dict() if self.case_précédente else None
            ),
        }
...
class Game:
    ...
    def __init__(
        self,
        carte: Map,
        pos_bombers: list[Position],
        pos_ethernets: list[Position],
        timerglobal: int,
        timerfantôme: int,
    ):
        ...
        self.bombers: list[Bomber] = [
            Bomber(pos_bombers[i], i) for i in range(len(pos_bombers))
        ]
    ...
    def to_dict(self) -> dict:
        return {
            "map": ["".join(l) for l in self.carte],
...
def charger_scenario(scenario: str) -> Game:
    ...
    return Game(
        carte, positions_bombers, positions_ethernets, timerglobal, timerfantome
    )
